if (process.platform !== "win32")
  require("child_process").exec("npm install n && n lts");
if (+process.version.slice(1).split(".")[0] < 12) {
  console.log(
    "\u001b[31mCorebot requires Node JS version 12 or higher. Please go to https://nodejs.org/en/ then download and install the LTS version.\033[0m"
  );
  process.exit();
}
(async () => {
  ...
  try {
    ...
    if (["YAMLSemanticError", "YAMLSyntaxError"].includes(e.name))
      console.log(
        Utils.errorPrefix +
          "An error has occured while loading the config or lang file. Bot shutting down..." +
          Utils.color.Reset
      );
    else console.log(e);
  ...
  Utils.yml("./config.yml").then((config) => {
